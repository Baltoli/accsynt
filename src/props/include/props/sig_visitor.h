...
class on {
  static_assert(
      std::is_invocable_v<F, param&> || std::is_invocable_v<F>,
      "Visitor function must be invocable with param or no args");

  static constexpr bool pass_param = std::is_invocable_v<F, param&>;
...
template <typename... Fs>
void sig_visitor<Fs...>::visit(signature& sig)
{
