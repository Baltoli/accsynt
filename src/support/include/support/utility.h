...
struct enum_iterator {
  size_t i;
  TIter iter;
  bool operator!=(const enum_iterator<TIter>& other) const
  {
...
template <typename T, typename TIter = decltype(adl_begin(std::declval<T>())),
...
constexpr auto enumerate(T&& iterable)
{
  struct iterable_wrapper {
    ...
    auto begin()
    {
      return detail::enum_iterator<TIter>{ 0, adl_begin(iterable) };
    }
