...
// If you do not want to use these functions inline (and instead want to link w/
// strsafe.lib), then #define STRSAFE_LIB before including this header file.
...
// Some functions always run inline because they use stdin and we want to avoid
// building multiple versions of strsafe lib depending on if you use msvcrt,
// libcmt, etc.
...
#define STRSAFE_E_END_OF_FILE                                                  \
  ((HRESULT)0x80070026L) // 0x26 =  38L = ERROR_HANDLE_EOF

// Flags for controling the Ex functions
//
//      STRSAFE_FILL_BYTE(0xFF)     0x000000FF  // bottom byte specifies fill
//      pattern
#define STRSAFE_IGNORE_NULLS                                                   \
  0x00000100 // treat null as TEXT("") -- don't fault on NULL buffers
#define STRSAFE_FILL_BEHIND_NULL                                               \
  0x00000200 // fill in extra space behind the null terminator
#define STRSAFE_FILL_ON_FAILURE                                                \
  0x00000400 // on failure, overwrite pszDest with fill pattern and null
             // terminate it
#define STRSAFE_NULL_ON_FAILURE                                                \
  0x00000800 // on failure, set *pszDest = TEXT('\0')
#define STRSAFE_NO_TRUNCATION                                                  \
  0x00001000 // instead of returning a truncated result, copy/append nothing to
             // pszDest and null terminate it
...
STRSAFEAPI StringCopyWorkerW(
    WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc);
STRSAFEAPI StringCopyExWorkerA(char* pszDest, size_t cchDest, size_t cbDest,
    const char* pszSrc, char** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCopyExWorkerW(WCHAR* pszDest, size_t cchDest, size_t cbDest,
    const WCHAR* pszSrc, WCHAR** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCopyNWorkerA(
    char* pszDest, size_t cchDest, const char* pszSrc, size_t cchSrc);
STRSAFEAPI StringCopyNWorkerW(
    WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc, size_t cchSrc);
STRSAFEAPI StringCopyNExWorkerA(char* pszDest, size_t cchDest, size_t cbDest,
    const char* pszSrc, size_t cchSrc, char** ppszDestEnd,
    size_t* pcchRemaining, unsigned long dwFlags);
STRSAFEAPI StringCopyNExWorkerW(WCHAR* pszDest, size_t cchDest, size_t cbDest,
    const WCHAR* pszSrc, size_t cchSrc, WCHAR** ppszDestEnd,
    size_t* pcchRemaining, unsigned long dwFlags);
STRSAFEAPI StringCatWorkerA(char* pszDest, size_t cchDest, const char* pszSrc);
STRSAFEAPI StringCatWorkerW(
    WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc);
STRSAFEAPI StringCatExWorkerA(char* pszDest, size_t cchDest, size_t cbDest,
    const char* pszSrc, char** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCatExWorkerW(WCHAR* pszDest, size_t cchDest, size_t cbDest,
    const WCHAR* pszSrc, WCHAR** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCatNWorkerA(
    char* pszDest, size_t cchDest, const char* pszSrc, size_t cchMaxAppend);
STRSAFEAPI StringCatNWorkerW(
    WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc, size_t cchMaxAppend);
STRSAFEAPI StringCatNExWorkerA(char* pszDest, size_t cchDest, size_t cbDest,
    const char* pszSrc, size_t cchMaxAppend, char** ppszDestEnd,
    size_t* pcchRemaining, unsigned long dwFlags);
STRSAFEAPI StringCatNExWorkerW(WCHAR* pszDest, size_t cchDest, size_t cbDest,
    const WCHAR* pszSrc, size_t cchMaxAppend, WCHAR** ppszDestEnd,
    size_t* pcchRemaining, unsigned long dwFlags);
...
string including the null terminator. The flags parameter allows additional
controls.
...
the number of characters left in the destination string, including the null
terminator
    ...
    dwFlags         -   controls some details of the string copy:
        ...
        STRSAFE_NULL_ON_FAILURE
                    ...
                    to the empty string. This will overwrite any truncated
string returned when the failure is STRSAFE_E_INSUFFICIENT_BUFFER.
...
STRSAFEAPI StringCchCopyExA(char* pszDest, size_t cchDest, const char* pszSrc,
    char** ppszDestEnd, size_t* pcchRemaining, unsigned long dwFlags);
STRSAFEAPI StringCchCopyExW(WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc,
    WCHAR** ppszDestEnd, size_t* pcchRemaining, unsigned long dwFlags);
#ifdef UNICODE
    ...
    char** ppszDestEnd, size_t* pcchRemaining, unsigned long dwFlags)
{
  ...
  if (cchDest > STRSAFE_MAX_CCH) {
    ...
    // safe to multiply cchDest * sizeof(char) since cchDest < STRSAFE_MAX_CCH
    // and sizeof(char) is 1
    cbDest = cchDest * sizeof(char);

    hr = StringCopyExWorkerA(
        pszDest, cchDest, cbDest, pszSrc, ppszDestEnd, pcchRemaining, dwFlags);
  }
    ...
    WCHAR** ppszDestEnd, size_t* pcchRemaining, unsigned long dwFlags)
{
  ...
  if (cchDest > STRSAFE_MAX_CCH) {
    ...
    // safe to multiply cchDest * sizeof(WCHAR) since cchDest < STRSAFE_MAX_CCH
    // and sizeof(WCHAR) is 2
...
Routine Description:
    ...
    including the null terminator. The flags parameter allows additional
controls.
...
Arguments:
    ...
    cbDest          -   size of destination buffer in bytes.
                        length must be ((_tcslen(pszSrc) + 1) * sizeof(TCHAR))
to hold all of the source including the null terminator
...
STRSAFEAPI StringCbCopyExA(char* pszDest, size_t cbDest, const char* pszSrc,
    char** ppszDestEnd, size_t* pcbRemaining, unsigned long dwFlags);
STRSAFEAPI StringCbCopyExW(WCHAR* pszDest, size_t cbDest, const WCHAR* pszSrc,
    WCHAR** ppszDestEnd, size_t* pcbRemaining, unsigned long dwFlags);
#ifdef UNICODE
    ...
    char** ppszDestEnd, size_t* pcbRemaining, unsigned long dwFlags)
{
  ...
  if (cchDest > STRSAFE_MAX_CCH) {
    ...
    hr = StringCopyExWorkerA(
        pszDest, cchDest, cbDest, pszSrc, ppszDestEnd, &cchRemaining, dwFlags);
  }

  if (SUCCEEDED(hr) || (hr == STRSAFE_E_INSUFFICIENT_BUFFER)) {
    if (pcbRemaining) {
      // safe to multiply cchRemaining * sizeof(char) since cchRemaining <
      // STRSAFE_MAX_CCH and sizeof(char) is 1
    ...
    WCHAR** ppszDestEnd, size_t* pcbRemaining, unsigned long dwFlags)
{
  ...
  if (SUCCEEDED(hr) || (hr == STRSAFE_E_INSUFFICIENT_BUFFER)) {
    if (pcbRemaining) {
      // safe to multiply cchRemaining * sizeof(WCHAR) since cchRemaining <
      // STRSAFE_MAX_CCH and sizeof(WCHAR) is 2
...
Routine Description:
    ...
    truncation and the resultant destination string was null terminated,
otherwise it will return a failure code. In failure cases as much of pszSrc will
be copied to pszDest as possible, and pszDest will be null terminated.
...
STRSAFEAPI StringCbCopyNA(
    char* pszDest, size_t cbDest, const char* pszSrc, size_t cbSrc);
STRSAFEAPI StringCbCopyNW(
    WCHAR* pszDest, size_t cbDest, const WCHAR* pszSrc, size_t cbSrc);
#ifdef UNICODE
...
STRSAFEAPI StringCchCopyNExA(char* pszDest, size_t cchDest, const char* pszSrc,
    size_t cchSrc, char** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCchCopyNExW(WCHAR* pszDest, size_t cchDest,
    const WCHAR* pszSrc, size_t cchSrc, WCHAR** ppszDestEnd,
    size_t* pcchRemaining, unsigned long dwFlags);
#ifdef UNICODE
    ...
    unsigned long dwFlags)
{
  ...
  if ((cchDest > STRSAFE_MAX_CCH) || (cchSrc > STRSAFE_MAX_CCH)) {
    ...
    hr = StringCopyNExWorkerA(pszDest, cchDest, cbDest, pszSrc, cchSrc,
    ...
    size_t* pcchRemaining, unsigned long dwFlags)
{
  ...
  if ((cchDest > STRSAFE_MAX_CCH) || (cchSrc > STRSAFE_MAX_CCH)) {
    ...
    hr = StringCopyNExWorkerW(pszDest, cchDest, cbDest, pszSrc, cchSrc,
        ppszDestEnd, pcchRemaining, dwFlags);
  }
...
STRSAFEAPI StringCbCopyNExA(char* pszDest, size_t cbDest, const char* pszSrc,
    size_t cbSrc, char** ppszDestEnd, size_t* pcbRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCbCopyNExW(WCHAR* pszDest, size_t cbDest, const WCHAR* pszSrc,
    size_t cbSrc, WCHAR** ppszDestEnd, size_t* pcbRemaining,
    unsigned long dwFlags);
#ifdef UNICODE
...
Routine Description:
    ...
    This function returns a hresult, and not a pointer.  It returns a S_OK
    if the string was concatenated without truncation and null terminated,
otherwise it will return a failure code. In failure cases as much of pszSrc will
be appended to pszDest as possible, and pszDest will be null terminated.
...
Arguments:
    ...
    cbDest      -  size of destination buffer in bytes.
                   length must be = ((_tcslen(pszDest) + _tcslen(pszSrc) + 1) *
sizeof(TCHAR) to hold all of the combine string plus the null terminator
...
terminator
    ...
    dwFlags         -   controls some details of the string copy:
        ...
        STRSAFE_NO_TRUNCATION
                    if the function returns STRSAFE_E_INSUFFICIENT_BUFFER,
pszDest will not contain a truncated string, it will remain unchanged.
...
STRSAFEAPI StringCchCatExA(char* pszDest, size_t cchDest, const char* pszSrc,
    char** ppszDestEnd, size_t* pcchRemaining, unsigned long dwFlags);
STRSAFEAPI StringCchCatExW(WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc,
    WCHAR** ppszDestEnd, size_t* pcchRemaining, unsigned long dwFlags);
#ifdef UNICODE
...
STRSAFEAPI StringCbCatExA(char* pszDest, size_t cbDest, const char* pszSrc,
    char** ppszDestEnd, size_t* pcbRemaining, unsigned long dwFlags);
STRSAFEAPI StringCbCatExW(WCHAR* pszDest, size_t cbDest, const WCHAR* pszSrc,
    WCHAR** ppszDestEnd, size_t* pcbRemaining, unsigned long dwFlags);
#ifdef UNICODE
...
Routine Description:
    ...
    This function will not write past the end of the destination buffer and it
will ALWAYS null terminate pszDest (unless it is zero length).
...
Arguments:
    ...
    cchDest         -   size of destination buffer in characters.
                        length must be (_tcslen(pszDest) + min(cchMaxAppend,
_tcslen(pszSrc)) + 1) to hold all of the combine string plus the null
                        terminator.
...
Arguments:
    ...
    cbDest          -   size of destination buffer in bytes.
                        length must be ((_tcslen(pszDest) + min(cbMaxAppend /
sizeof(TCHAR), _tcslen(pszSrc)) + 1) * sizeof(TCHAR) to hold all of the combine
...
STRSAFEAPI StringCbCatNA(
    char* pszDest, size_t cbDest, const char* pszSrc, size_t cbMaxAppend);
STRSAFEAPI StringCbCatNW(
    WCHAR* pszDest, size_t cbDest, const WCHAR* pszSrc, size_t cbMaxAppend);
#ifdef UNICODE
...
STRSAFEAPI StringCchCatNExA(char* pszDest, size_t cchDest, const char* pszSrc,
    size_t cchMaxAppend, char** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCchCatNExW(WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc,
    size_t cchMaxAppend, WCHAR** ppszDestEnd, size_t* pcchRemaining,
    unsigned long dwFlags);
#ifdef UNICODE
    ...
    unsigned long dwFlags)
{
  ...
  if (cchDest > STRSAFE_MAX_CCH) {
    ...
    hr = StringCatNExWorkerA(pszDest, cchDest, cbDest, pszSrc, cchMaxAppend,
        ppszDestEnd, pcchRemaining, dwFlags);
  }
    ...
    unsigned long dwFlags)
{
  ...
  if (cchDest > STRSAFE_MAX_CCH) {
    ...
    hr = StringCatNExWorkerW(pszDest, cchDest, cbDest, pszSrc, cchMaxAppend,
        ppszDestEnd, pcchRemaining, dwFlags);
  }
...
string plus the null terminator.
    ...
    pcbRemaining    -   if pcbRemaining is non-null, the function will return
the number of bytes left in the destination string, including the null
terminator
...
STRSAFEAPI StringCbCatNExA(char* pszDest, size_t cbDest, const char* pszSrc,
    size_t cbMaxAppend, char** ppszDestEnd, size_t* pcbRemaining,
    unsigned long dwFlags);
STRSAFEAPI StringCbCatNExW(WCHAR* pszDest, size_t cbDest, const WCHAR* pszSrc,
    size_t cbMaxAppend, WCHAR** ppszDestEnd, size_t* pcbRemaining,
    unsigned long dwFlags);
#ifdef UNICODE
...
condition, use feof or ferror to determine which one has occurred.

    STRSAFE_E_INSUFFICIENT_BUFFER /
    HRESULT_CODE(hr) == ERROR_INSUFFICIENT_BUFFER
                -   this return value is an indication that there was
insufficient space in the destination buffer to copy any data
...
Notes:
    pszDest should not be NULL unless the STRSAFE_IGNORE_NULLS flag is
specified. If STRSAFE_IGNORE_NULLS is passed and pszDest is NULL, an error may
still be returned even though NULLS are ignored
...
Arguments:
    ...
    pcch        -   if the function succeeds and pcch is non-null, the current
length in characters of psz excluding the null terminator will be returned. This
out parameter is equivalent to the return value of strlen(psz)
...
Return Value:
    ...
    S_OK        -   psz is non-null and the length including the null terminator
is less than or equal to cchMax characters
...
Arguments:
    ...
    pcb         -   if the function succeeds and pcb is non-null, the current
length in bytes of psz excluding the null terminator will be returned. This out
parameter is equivalent to the return value of strlen(psz) * sizeof(TCHAR)

Notes:
    psz can be null but the function will fail

    cbMax should be greater than or equal to sizeof(TCHAR) or the function will
fail

Return Value:

    S_OK        -   psz is non-null and the length including the null terminator
is less than or equal to cbMax bytes
...
STRSAFEAPI StringCbLengthA(const char* psz, size_t cbMax, size_t* pcb)
{
  ...
  if (SUCCEEDED(hr) && pcb) {
    // safe to multiply cch * sizeof(char) since cch < STRSAFE_MAX_CCH and
    // sizeof(char) is 1
    *pcb = cch * sizeof(char);
  }
...
STRSAFEAPI StringCbLengthW(const WCHAR* psz, size_t cbMax, size_t* pcb)
{
  ...
  if (SUCCEEDED(hr) && pcb) {
    // safe to multiply cch * sizeof(WCHAR) since cch < STRSAFE_MAX_CCH and
    // sizeof(WCHAR) is 2
...
STRSAFEAPI StringCatWorkerA(char* pszDest, size_t cchDest, const char* pszSrc)
{
  ...
  if (SUCCEEDED(hr)) {
    hr = StringCopyWorkerA(
        pszDest + cchDestCurrent, cchDest - cchDestCurrent, pszSrc);
  }
  ...
  if (dwFlags & (~STRSAFE_VALID_FLAGS)) {
    ...
    if (SUCCEEDED(hr)) {
      if (cchDest == 0) {
        ...
        // we handle the STRSAFE_FILL_ON_FAILURE and STRSAFE_NULL_ON_FAILURE
        // cases below, so do not pass those flags through
    ...
    if (SUCCEEDED(hr)) {
      if (cchDest == 0) {
        ...
        hr = StringCopyExWorkerW(pszDestEnd, cchRemaining,
            (cchRemaining * sizeof(WCHAR)) + (cbDest % sizeof(WCHAR)), pszSrc,
            &pszDestEnd, &cchRemaining,
            dwFlags & (~(STRSAFE_FILL_ON_FAILURE | STRSAFE_NULL_ON_FAILURE)));
      }
    }
    ...
    char* pszDest, size_t cchDest, const char* pszSrc, size_t cchMaxAppend)
{
  ...
  if (SUCCEEDED(hr)) {
    hr = StringCopyNWorkerA(pszDest + cchDestCurrent, cchDest - cchDestCurrent,
        pszSrc, cchMaxAppend);
  }
    ...
    WCHAR* pszDest, size_t cchDest, const WCHAR* pszSrc, size_t cchMaxAppend)
{
  ...
  if (SUCCEEDED(hr)) {
    hr = StringCopyNWorkerW(pszDest + cchDestCurrent, cchDest - cchDestCurrent,
        pszSrc, cchMaxAppend);
  }
