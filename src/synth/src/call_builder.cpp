...
bool call_builder::operator==(call_builder const& other) const
{
  ...
  for(auto const& param : signature_.parameters) {
    ...
    if(param.pointer_depth == 0) {
      ...
      // Pointer comparisons need to look at the data stored
      if(param.type == data_type::integer) {
        all_eq = all_eq && 
          (int_data_.at(int_data_offset) == other.int_data_.at(int_data_offset));
      } else if(param.type == data_type::floating) {
        all_eq = all_eq && 
          (float_data_.at(float_data_offset) == other.float_data_.at(float_data_offset));
      }
    }
  }
